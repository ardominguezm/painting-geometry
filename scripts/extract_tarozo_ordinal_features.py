...
TIE_CODES = np.asarray(
    [p[0] * 64 + p[1] * 16 + p[2] * 4 + p[3] for p in TIE_PATTERNS],
    dtype=np.int16,
)
...
def build_ordinal_maps():
    """Use ordpy once on canonical patterns to reproduce its grouping and tie-resolution maps exactly."""
    ...
    for pat, key in zip(TIE_PATTERNS, TIE_PATTERN_STRINGS):
        ...
        grouped = ordpy.two_by_two_patterns(
            arr,
            taux=1,
            tauy=1,
            overlapping=True,
        ...
        if not np.isclose(sum(grouped.values()), 1.0):
            raise RuntimeError(f"Unexpected grouped ordinal distribution for {key}: {grouped}")
        group_of[key] = str(max(grouped, key=grouped.get))

        standard = ordpy.two_by_two_patterns(
            arr,
            taux=1,
            tauy=1,
            overlapping=True,
        ...
        # ordpy returns the COMPLETE 24-pattern dictionary, with one entry equal to 1
        # for a single 2x2 patch and the remaining entries equal to 0.  Therefore the
        # tie-resolved standard pattern is the argmax, not the sole dictionary key.
        if len(standard) != 24 or not np.isclose(sum(standard.values()), 1.0):
            raise RuntimeError(f"Unexpected standard ordinal distribution for {key}: {standard}")
        standard_of[key] = str(max(standard, key=standard.get))
...
def dense_rank_codes_2x2(gray: np.ndarray, tie_precision: int | None = None) -> np.ndarray:
    """Vectorized dense ranks for every overlapping 2x2 patch, matching tie-aware ordpy ranking."""
    a = np.asarray(gray)
...
def fast_p75(gray: np.ndarray, tie_precision: int | None = None) -> dict[str, float]:
    ...
    return {
        key: float(counts[int(code)] / denom)
        for key, code in zip(TIE_PATTERN_STRINGS, TIE_CODES)
    }
...
def aggregate_from_p75(p75: dict[str, float]):
    ...
    for key, prob in p75.items():
        ...
        p24[STANDARD_OF[key]] += float(prob)
    return p11, p24


def validate_fast_implementation() -> None:
    """Exact-equivalence tests against ordpy before processing the corpus."""
    rng = np.random.default_rng(20260826)
    for case in range(4):
        # Integer-valued images create many ties and therefore provide a stringent test.
        ...
        slow75.update({str(k): float(v) for k, v in slow75_raw.items()})
        ...
        if not np.allclose(f75, s75, atol=1e-12, rtol=0):
            raise RuntimeError(
                f"Fast OP75 validation failed in synthetic case {case}; "
                f"max diff={np.max(np.abs(f75 - s75))}"
            )
        ...
        slow24.update({str(k): float(v) for k, v in slow24_raw.items()})
...
def extract_one(gray: np.ndarray, tie_precision=None) -> dict[str, float]:
    ...
    tie_mass = sum(
        ...
        if len(set(sanitize_pattern_key(pattern))) < 4
    )
...
def normalize_checkpoint(df: pd.DataFrame) -> pd.DataFrame:
    """Old checkpoints may have sparse columns; unobserved ordinal probabilities are zeros."""
    if df.empty:
        return df
...
def main(
    ...
    if not hasattr(ordpy, "two_by_two_patterns"):
        raise RuntimeError(
            f"Installed ordpy {getattr(ordpy, '__version__', 'unknown')} lacks "
            "two_by_two_patterns; require >=1.2.0"
        )

    # Scientific guardrail: the accelerated implementation must reproduce ordpy exactly.
    ...
    if missing:
        raise ValueError(
            f"Feature matrix missing required metadata columns: {sorted(missing)}"
        )
    ...
    if checkpoint.exists():
        ...
        print(
            f"RESUME: loaded {len(completed)} completed paintings from {checkpoint}"
        )
    ...
    for i in tqdm(pending, total=len(pending), desc="Fast Tarozo ordinal features"):
        ...
        if checkpoint_every > 0 and since_save >= checkpoint_every:
            ck = normalize_checkpoint(
                pd.DataFrame([completed[k] for k in sorted(completed)])
            )
    ...
    if failures:
        pd.DataFrame(failures).to_csv(failure_path, index=False)
        print(
            f"WARNING: {len(failures)} failures. "
            "Rows with failures will be dropped from enriched output."
        )
    ...
    enriched = pd.concat(
        [
            base.loc[keep_idx].reset_index(drop=True),
            ord_df.loc[keep_idx].reset_index(drop=True),
        ],
        axis=1,
    )
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Extract Tarozo et al. two-by-two ordinal-pattern features for the ArtBench pilot."
    )
    p.add_argument(
        "--features",
        type=Path,
        required=True,
        help="Phase-IV artbench_pilot_features.csv",
    )
    p.add_argument(
        "--dataset-root",
        type=Path,
        required=True,
        help="Directory containing extracted ArtBench ImageFolder",
    )
